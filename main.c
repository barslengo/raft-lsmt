...
#define GET_REQUEST_CODE 1
#define INSERT_REQUEST_CODE 2
...
typedef struct
{
  ...
  /* the responding node's client port */
  int client_port;
...
typedef struct
{
  ...
  uv_loop_t peer_loop, client_loop;
...
static void __on_client_connection(uv_stream_t *listener, const int status);
...
/**
 * Entry point for the thread that will run the client-facing event loop.
 */
static void __client_loop_runner(void* arg)
{
  /* sv is global, so arg is not needed. */
  uv_run(&sv->client_loop, UV_RUN_DEFAULT);
}

/**
 * A simple callback to free the uv_write_t request after a write is done.
 */
static void __on_client_write_done(uv_write_t* req, int status)
{
  uint8_t *data_buff = (uint8_t *)req->data;
...
/* Initializes the client-facing TCP listener and runs its event loop in a new thread. */
static void __start_client_socket_threaded(const char* host, int port, uv_tcp_t* listen)
{
  int e;

  memset(&sv->client_loop, 0, sizeof(uv_loop_t));
  e = uv_loop_init(&sv->client_loop);
  if (0 != e)
    uv_fatal(e);

  uv_bind_listen_socket(listen, host, port, &sv->client_loop);
  e = uv_listen((uv_stream_t*)listen, MAX_CLIENT_CONNECTIONS, __on_client_connection);
  if (0 != e)
    uv_fatal(e);

  uv_thread_t client_thread_id;
  uv_thread_create(&client_thread_id, __client_loop_runner, NULL);
}


/**
 * Callback executed on the client_loop when a client sends data.
 * This function contains the ticket generation logic.
 */
static void __client_read_cb(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
  if (nread < 0) {
    if (nread != UV_EOF)
  ...
  /* Any data received is a signal to generate a ticket. We can free the buffer. */

  //TODO: if it's a GET request then just fetch the db, otherwise insert and sync with raft nodes.
  if (buf->base[0] == GET_REQUEST_CODE) {
    sl_uint128_t key = *((sl_uint128_t*)(buf->base+1));
    uint8_t *content = lsmt_get(sv->db, key);

    if (buf->base) free(buf->base);
    if (content) {
      uint32_t content_length = *((uint32_t *)(content+1));
      uv_buf_t res_buf = uv_buf_init((char *)content, 1 + 4 + content_length);
      uv_write_t *write_req = calloc(1, sizeof(uv_write_t));
      uv_write(write_req, stream, &res_buf, 1, __on_client_write_done);
      uv_close((uv_handle_t*)stream, (uv_close_cb)free);
      return;
    }
    else {
    ...
    //uint8_t content_length = *(uint32_t *)(buf->base+1+sizeof(sl_uint128_t)+1);
    //uint8_t *content = (uint8_t *)(buf->base+1+sizeof(sl_uint128_t));
    //lsmt_insert(sv->db, key, content, content_length + 1 + 4);
  }

  if (buf->base[0] != INSERT_REQUEST_CODE) {
    if (buf->base) free(buf->base);
    return;
  }
  ...
  if (raft_node_get_id(leader) != sv->node_id)
  {
    ...
    char leader_info[128];
    snprintf(leader_info, sizeof(leader_info), "REDIRECT %s:%d\n",
             inet_ntoa(leader_conn->addr.sin_addr),
             leader_conn->client_port);

    uv_mutex_unlock(&sv->raft_lock);

    uv_buf_t res_buf = uv_buf_init(leader_info, strlen(leader_info));
    uv_write_t *write_req = calloc(1, sizeof(uv_write_t));
    uv_write(write_req, stream, &res_buf, 1, __on_client_write_done);
    uv_close((uv_handle_t*)stream, (uv_close_cb)free);
  ...
  entry.data.buf = (void *)(buf->base+1);
  entry.data.len = buf->len-1; 
  ...
  if (0 != e) {
    ...
    if (buf->base) free(buf->base);
    //TODO: return error msg to the client.
    return;
  }
  ...
  /* Block this client thread until the entry is committed */
  int done = 0, tries = 0;
  //TODO: return error msg to the client instead of just do breaks.
  do {
    if (3 < tries) {
      fprintf(stderr, "ERROR: failed to commit entry after multiple attempts\n");
      fflush(stdout);
      break;
    }
  ...
  if (done) {
    ...
    char resp[] = "INSERT OK\n";
    uv_buf_t res_buf = uv_buf_init(resp, sizeof(resp)); 
    uv_write_t *write_req = calloc(1, sizeof(uv_write_t));
    uv_write(write_req, stream, &res_buf, 1, __on_client_write_done);
    /*
    char ticket_str[32];
    snprintf(ticket_str, sizeof(ticket_str), "%u\n", ticket);
    uv_buf_t res_buf = uv_buf_init(ticket_str, strlen(ticket_str));
    uv_write_t *write_req = calloc(1, sizeof(uv_write_t));
    uv_write(write_req, stream, &res_buf, 1, __on_client_write_done);
    */
  }
...
/**
 * Callback executed on the client_loop when a new client connects.
 * Mirrors the design of __on_peer_connection.
 */
static void __on_client_connection(uv_stream_t *listener, const int status)
{
  int e;
  ...
  e = uv_tcp_init(listener->loop, client);
  if (0 != e) {
    free(client);
    uv_fatal(e);
  }

  e = uv_accept(listener, (uv_stream_t*)client);
  if (0 != e) {
    uv_close((uv_handle_t*)client, (uv_close_cb)free);
    uv_fatal(e);
  }

  e = uv_read_start((uv_stream_t*)client, __peer_alloc_cb, __client_read_cb);
  if (0 != e)
    uv_fatal(e);
}
                              ...
                              raft_logtype_e change_type)
{
  ...
  conn->client_port = change->client_port;
  ...
  fflush(stdout);
  if (conn->node) {
    printf("node_id=%lu", raft_node_get_id(conn->node));
    fflush(stdout);
  }
  ...
  if (conn->node) raft_node_set_udata(conn->node, conn);
...
)
{
  ...
  sl_uint128_t key = *((sl_uint128_t*)ety->data.buf);
  uint32_t content_size = *((uint32_t *)(ety->data.buf+sizeof(sl_uint128_t)+1));
  uint8_t *content = (uint8_t *)(ety->data.buf + sizeof(sl_uint128_t) + 1 + 4 + content_size);

  //MDB_val key = { .mv_size = ety->data.len, .mv_data = ety->data.buf };
  //MDB_val val = { .mv_size = 0, .mv_data = "\0" };

  int e = mdb_txn_begin(sv->raft_db_env, NULL, 0, &txn);
  if (0 != e)
    mdb_fatal(e);
  ...
  //TODO: replace with lsmt insert.
  e = lsmt_insert(sv->db, key, content, 1 + 4 + content_size);
...
{
  return mdb_puts_int_commit(sv->raft_db_env, sv->state, "term", current_term);
}
...
{
  return mdb_puts_int_commit(sv->raft_db_env, sv->state, "voted_for", voted_for);
}
...
static int __deserialize_and_handle_msg(void *img, size_t sz, void *data)
{
  ...
  switch (m.type)
  {
    case MSG_HANDSHAKE:
      {
        ...
        conn->client_port = m.hs.client_port;
        ...
        else
      {
          int e = __append_cfg_change(sv, RAFT_LOGTYPE_ADD_NONVOTING_NODE,
                                      ...
                                      m.hs.raft_port, m.hs.client_port,
                                      m.hs.node_id);
          if (0 != e)
            return __send_handshake_response(conn, HANDSHAKE_FAILURE, NULL);
          return __send_handshake_response(conn, HANDSHAKE_SUCCESS, NULL);
    ...
    case MSG_HANDSHAKE_RESPONSE:
      if (0 == m.hsr.success)
      {
        conn->client_port = m.hsr.client_port;
...
static void __send_handshake(peer_connection_t* conn)
{
  ...
  msg.hs.client_port = atoi(opts.http_port);
                                     ...
                                     raft_node_t* leader)
{
  ...
  msg.hsr.client_port = atoi(opts.http_port);
...
)
{
  ...
  e = mdb_txn_begin(sv->raft_db_env, NULL, 0, &txn);
  if (0 != e)
    mdb_fatal(e);
...
)
{
  ...
  mdb_poll(sv->raft_db_env, sv->entries, &k, &v);
...
)
{
  ...
  mdb_pop(sv->raft_db_env, sv->entries, &k, &v);
...
static void __load_commit_log(server_t* sv)
{
  ...
  e = mdb_txn_begin(sv->raft_db_env, NULL, MDB_RDONLY, &txn);
  if (0 != e)
    mdb_fatal(e);
  ...
  MDB_val val;
  mdb_gets(sv->raft_db_env, sv->state, "commit_idx", &val);
  if (val.mv_data)
...
static void __load_persistent_state(server_t* sv)
{
  ...
  mdb_gets_int(sv->raft_db_env, sv->state, "voted_for", &val);
  raft_vote_for_nodeid(sv->raft, val);
  mdb_gets_int(sv->raft_db_env, sv->state, "term", &val);
...
static int __load_opts(server_t* sv, options_t* opts)
{
  ...
  int e = mdb_gets_int(sv->raft_db_env, sv->state, "id", &sv->node_id);
  if (-1 == e)
    return e;

  e = mdb_gets_int(sv->raft_db_env, sv->state, "client_port", &client_port);
  if (-1 == e)
    abort();

  e = mdb_gets_int(sv->raft_db_env, sv->state, "raft_port", &raft_port);
  if (-1 == e)
    abort();

  free(opts->http_port);
  free(opts->raft_port);
  asprintf(&opts->http_port, "%d", client_port);
...
static void __drop_db(server_t* sv)
{
  MDB_dbi dbs[] = { sv->entries, sv->state };
  mdb_drop_dbs(sv->raft_db_env, dbs, len(dbs));
  lsmt_free(sv->db); 
}
...
{
  mdb_db_env_create(&sv->raft_db_env, 0, opts.path, atoi(opts.db_size));
  mdb_db_create(&sv->entries, sv->raft_db_env, "entries");
  //mdb_db_create(&sv->tickets, sv->raft_db_env, "docs");
  mdb_db_create(&sv->state, sv->raft_db_env, "state");
  sv->db = lsmt_init();
}


/*
static void __start_client_socket(server_t* sv, const char* host, int port, uv_tcp_t* listen)
{
...
{
  mdb_puts_int_commit(sv->raft_db_env, sv->state, "id", sv->node_id);
  mdb_puts_int_commit(sv->raft_db_env, sv->state, "raft_port", atoi(opts->raft_port));
  mdb_puts_int_commit(sv->raft_db_env, sv->state, "client_port", atoi(opts->http_port));
}
...
int main(int argc, char **argv)
{
  ...
  uv_tcp_t client_listen, peer_listen;
  ...
  else
{
  ...
  if (opts.start || opts.join)
  {
    ...
    __start_client_socket_threaded(opts.host, atoi(opts.http_port), &client_listen);
