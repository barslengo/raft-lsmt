...
class TestWriteBenchmarkMulti(unittest.TestCase):
    def setUp(self):
        ...
        for csv_file in glob.glob("client_throughput_*.csv"):
            try:
                os.remove(csv_file)
            except Exception:
                pass
    ...
    def test_multi_benchmark_execution(self):
        # Execute the run_bench_multi.sh script as a subprocess
        # We run with 3 workers and a total of 15000 requests (5000 each)
        cmd = [
            "./bash-scripts/run_bench_multi.sh",
        ...
        self.assertEqual(result.returncode, 0, f"Multi benchmark script failed with code {result.returncode}")
        
        # Verify that multiple CSV files were generated, one for each worker, and one merged CSV
        all_csv_files = glob.glob("client_throughput_*.csv")
        ...
        for csv_file in all_csv_files:
            ...
            self.assertEqual(header, "Timestamp,Total_ACKed_Records,Total_ACKed_Bytes,OPS,MBps")
